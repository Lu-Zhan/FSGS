...
def read_json(json_path):
    ...
    psnr = results["ours_30000"]["PSNR"]
    ssim = results["ours_30000"]["SSIM"]
...
def stat_mip_results(exp_names, method_names, scene_names):
    ...
    for exp_name in exp_names:
        ...
        for method_name in method_names:
            for scene_name in scene_names:
                metric_dir = os.path.join("/home/titan/exps/exps_frgs/byproduct", exp_name, method_name, scene_name, "metrics")
...
def stat_json(exp_names, method_names, scene_names):
    for exp_name in exp_names:
        ...
        for method_name in method_names:
            for scene_name in scene_names:
                results_json = os.path.join("/home/titan/exps/exps_frgs/byproduct", exp_name, method_name, scene_name, "results.json")
                psnr, ssim, lpips = read_json(results_json)

                if method_name not in res_dict.keys():
                    res_dict[method_name] = {scene_name: {"psnr": psnr, "ssim": ssim, "lpips": lpips}}
                else:
                    res_dict[method_name][scene_name] = {"psnr": psnr, "ssim": ssim, "lpips": lpips}

                print(f"{method_name} {scene_name} PSNR: {psnr:.4f} SSIM: {ssim:.4f}, LPIPS: {lpips:.4f}")
...
if __name__ == "__main__":
    ...
    stat_mip_results(exp_names, method_names, scene_names)
