...
def render_set(model_path, name, iteration, views, gaussians, pipeline, background, args, scale_factor):
...
def render_sets(dataset : ModelParams, pipeline : PipelineParams, args):
    sf = {'images': 1, 'images_2': 2, 'images_4': 4, 'images_8': 8}
    ...
    with torch.no_grad():
        ...
        if not args.skip_train:
            render_set(dataset.model_path, "train", scene.loaded_iter, scene.getTrainCameras(), gaussians, pipeline, background, args, scale_factor=scale_factor)
        if not args.skip_test:
            render_set(dataset.model_path, "test", scene.loaded_iter, scene.getTestCameras(), gaussians, pipeline, background, args, scale_factor=scale_factor)
